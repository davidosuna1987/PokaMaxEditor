...
export default class StepsWizard {
  constructor(element = null, options = {}) {
    ...
    this.next_btns = element.querySelectorAll(this.options.next_selector);
    this.nav_to_btns = element.querySelectorAll(this.options.nav_to_selector);
  ...
  bind() {
    ...
    if (this.next_btns != null) {
      MOUSE_EVENTS.forEach((event) => {
        this.next_btns.forEach((cnb) => {
          cnb.addEventListener(event, function(e) {
            e.preventDefault();
            _this.next_step();
          });
        });
    ...
    if (this.nav_to_btns != null) {
      MOUSE_EVENTS.forEach((event) => {
        this.nav_to_btns.forEach((ntb) => {
          ntb.addEventListener(event, function(e) {
            e.preventDefault();
            _this.go_to_step(ntb);
          });
        });
      });
    }
  ...
  go_to_step(ntb) {
    var target_step = parseInt(ntb.getAttribute('data-nav-to'));

    if (target_step == null || isNaN(target_step)) {
      return;
    }

    if (target_step >= this.steps.length) {
      if (typeof this.options.onFinish != 'undefined' && this.options.onFinish != null && this.options.onFinish) {
        this.options.onFinish(target_step);
      }
      this.deactivate_step(target_step);
    } else {
      for (var i = 0; i < this.steps.length; i++) {
          var stepID = this.steps[i].getAttribute('data-step-id');

          if(stepID < target_step){
            this.complete_step(stepID);
          } else if(stepID >= target_step){
            this.uncomplete_step(stepID);
          }
      }

      this.activate_step(target_step);
    }
  }
